...
다크 모드를 기본값으로 하는 글래스모피즘 디자인 시스템입니다. 프레임워크 없이 `css/style.css` 한 파일에
토큰(색/반경/그림자/이징)과 컴포넌트가 모두 정의돼 있습니다.

- **듀얼 테마**: 첫 방문은 `prefers-color-scheme`을 따르고, 사용자가 고르면 `localStorage`(`joygolf-theme`)에
  저장됩니다. 각 페이지 `<head>`의 인라인 스크립트가 페인트 전에 테마를 반영해 깜빡임(FOUC)이 없습니다.
- **레이아웃**: 12칼럼 벤토 그리드(`.bento` + `.col-3` ~ `.col-12`). 카드는 내용 높이에 맞춰지므로
  (`align-items: start`) 짧은 카드 아래에 빈 공간이 남지 않습니다.
- **네비게이션**: 데스크톱은 상단에 고정된 글래스 바 + 알약 링크. 모바일(860px 이하)은 하단 탭바(홈·연습·
  스코어·모임)로 바뀌고, 나머지 메뉴는 "더보기" 버튼을 누르면 바텀시트로 열립니다.
- **모션**: 카드 진입 시 순차 페이드업(`JoyGolf.revealCards()`가 `--i`로 지연 시간을 매김), 스프링 이징
  호버, 레벨 게이지 shimmer — `prefers-reduced-motion`을 존중해 자동으로 비활성화됩니다.
- **타이포**: Pretendard Variable (CDN), 한글 가독성을 위한 tight tracking
- 육각형 레이더 차트와 스코어 추이 라인 차트는 테마 전환 시 색상을 다시 계산해 그려집니다.
...
js/nav.js           공통 네비게이션 (상단바 + 모바일 하단 탭바/더보기 시트)
